...
The dataset used can be found [on my Github]().
...
Let's just concern ourselves with on predictor to start - we will relate TV advertising to Sales.  Let's plot the data:
...
This tells us that in addition to the independent effects of TV and Radio, every thousand dollars spent on radio further boost sales driven by TV, and that this effect is significant.  Further, the $R^{2}$ of this model is `r round(summary(model_interact)$r.squared, 2)`, which is even better than our prior model.
...
These issues artifically inflate the variance of a dataset, which can render estimation inaccurate.

*Collinearity* is when two inputs are highly correlated (eg > 0.5).  This can be identified with a simple `cor()` command and one of these inputs should be consider for removal:
...
*Multi-collinearity* is when more than two inputs are linearly related, and this may not be seen in a simple pairwise `cor()` command.  For this, you need to compute a *Variance Inflation Factor* (VIF) on a model with all your inputs and consider dropping inputs with a VIF greater than 5.
